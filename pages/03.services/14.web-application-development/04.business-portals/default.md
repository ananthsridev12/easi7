...
capabilities:
    ...
    - { title: 'Document & Data Sharing Workflows', items: ['Structured document exchange instead of email attachments', 'Version control so everyone works from the current file'] }
    - { title: 'Integration With Internal Systems', items: ['Live connection to the internal systems the shared data actually lives in', 'No manually maintained duplicate of internal records'] }
    - { title: 'Usage Analytics', items: ['Visibility into which partners or teams actually use the portal', 'Data to identify who needs more onboarding support'] }
deliverables:
...
who_needs_this:
    - { title: 'Businesses coordinating with more than a handful of partners or vendors', body: 'Email coordination works fine at small scale and breaks down predictably once the number of relationships grows.' }
    - { title: 'Organisations where internal teams currently rely on shared drives and spreadsheets', body: 'The same access and version-control problems that affect external partners often exist internally too.' }
faqs:
