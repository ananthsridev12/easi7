...
why_it_matters: "<p>Most sites don't need what a database-backed CMS provides - real-time editing by multiple non-technical users, dynamic per-request content, complex user permissions. For a site that's mostly informational or updated infrequently by a Git-comfortable team, that machinery is overhead adding attack surface and hosting cost without adding value. A static site removes an entire category of security risk since there's no database to inject into and no server-side script to exploit, and it's typically faster too, because the page was already built before anyone visited.</p>"
challenges:
