...
capabilities:
    ...
    - { title: 'Measurement Plan Documentation', items: ['A written plan mapping each KPI to its data source and owner', 'Kept current as the business changes'] }
    - { title: 'Stakeholder Alignment', items: ['Sign-off from whoever owns each decision the metric informs', "Resolving disagreement about what \"success\" means before it gets baked into a dashboard"] }
    - { title: 'Ongoing Recalibration', items: ['Scheduled reviews as the business model or funnel shifts', 'Retiring metrics that stopped mattering'] }
deliverables:
...
process:
    ...
    - { title: 'Measurement Plan Documentation', body: 'The mapping is written down, with a data source and owner assigned to each metric.' }
...
tools: ['Measurement Strategy', 'KPI Definition', 'Stakeholder Workshops']
outcomes:
    - 'Dashboards get checked regularly because they answer questions stakeholders actually asked'
    - 'Fewer disagreements about what a metric means, because it was defined and signed off before anyone saw a number'
    - 'Tracking implementation starts with a clear brief instead of guessing what might be useful later'
who_needs_this:
    - { title: 'Teams with dashboards nobody opens', body: 'If reporting exists but nobody checks it, the problem is usually that it was never tied to a decision in the first place.' }
    - { title: 'Businesses about to invest in new tracking or a data platform', body: 'Defining what needs measuring before the build starts is cheaper than re-architecting after.' }
faqs:
