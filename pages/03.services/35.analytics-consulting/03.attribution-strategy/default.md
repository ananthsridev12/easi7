...
sitemap:
    changefreq: monthly
    priority: !!float 0.6
definition: "<p>We answer the decision above attribution modelling: before any model gets implemented, which one is actually defensible for your business, given its funnel length, channel mix, and how much of the buyer journey happens somewhere trackable at all. There's no universally correct model - only the one that best fits your specific business, chosen deliberately rather than left at whatever a platform defaults to.</p>"
...
why_it_matters: "<p>Every attribution model encodes an assumption about how credit should be split - last-click assumes the final touch did all the work, linear assumes every touch mattered equally - and applying the wrong one doesn't produce an obviously wrong number, it produces a plausible-looking one that quietly misdirects budget toward whichever channel that model happens to favour. Getting this wrong is expensive precisely because it doesn't look wrong: a channel doing real work higher in the funnel gets defunded because the model structurally can't see its contribution, and nobody questions the number because it came out of the analytics platform.</p>"
challenges:
