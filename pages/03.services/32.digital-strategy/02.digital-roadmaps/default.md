...
service_url: /services/digital-strategy/digital-roadmaps
taxonomy:
    service: ['Digital Roadmaps']
metadata:
    description: 'Digital roadmap consulting services sequencing initiatives by dependency, impact, and resourcing.'
sitemap:
    changefreq: monthly
    priority: !!float 0.6
...
faqs:
    ...
    - { q: 'What if a new, urgent initiative comes up mid-roadmap?', a: "That's normal and the roadmap should have a defined process for it - typically requiring the new initiative to be scored and checked against existing dependencies before it displaces something already scheduled, rather than jumping the queue by default." }
    - { q: 'Can you guarantee the roadmap timeline will hold exactly as planned?', a: "No - a roadmap is a plan built on the best information available at the time, and conditions change: a key hire falls through, a dependency turns out to be more complex than scoped. What we can guarantee is that the sequencing logic stays sound, so when something shifts, the plan can be adjusted without starting over." }
    - { q: 'Do you help execute the initiatives on the roadmap, or only build the plan?', a: "Both are available and scoped separately. Some clients take the roadmap and dependency map and execute internally. Others want us to carry specific initiatives through delivery. We're explicit about the split before starting." }
    - { q: 'How many initiatives can realistically be on one roadmap?', a: "There's no fixed number - it depends on team capacity, not ambition. Part of the value of the exercise is confronting how much can genuinely run in parallel, which is usually fewer initiatives than the initial wishlist assumed." }
cta:
    heading: 'Backlog longer than your team can realistically execute?'
    body: "We'll map the dependencies and give you an order that actually holds up."
    button_label: 'Talk to us'
    button_url: /contact
---

A roadmap that lists everything as high priority isn't a roadmap - it's a wishlist with a timeline attached. The actual work of roadmapping is deciding what genuinely has to happen before something else can, and being honest about what a team can carry in parallel without everything slowing down at once.

We start by inventorying every initiative already in motion or proposed, often surfacing duplicate or conflicting work nobody had connected yet. From there, dependencies get mapped, effort gets scored against real capacity, and the sequence gets built around what the map actually shows - not around which stakeholder pushed hardest for their initiative to go first.
